...
*   **Imported netlists (`netlist:` key):** a testbench may set `netlist: <file>` (under `tb/`) to use an existing SPICE deck instead of netlisting a `.sch` via xschem (`safe_tb_file`). The ngspice path runs the imported deck through the same `_finalize_netlist`, so managed capture is identical; vacask reads the `.sim` verbatim and captures from the `.raw` as usual. `--templates-dir` still short-circuits before the engine, so it overrides a per-testbench `netlist:`.
