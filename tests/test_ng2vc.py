...
"""Tests for the hardened ng2vc converter resolution/invocation (simulator.py).
...
def test_explicit_setting_wins(monkeypatch, tmp_path) -> None:
    monkeypatch.setattr(simulator.shutil, "which", lambda _n: None)
    cfg = {"ng2vc_binary": str(tmp_path / "custom_ng2vc.py")}
    assert simulator._resolve_ng2vc(cfg) == str(tmp_path / "custom_ng2vc.py")


def test_discovers_on_path(monkeypatch, tmp_path) -> None:
    found = _write(tmp_path / "ng2vc.py", "x")
    monkeypatch.setattr(simulator.shutil, "which",
                        lambda n: found if n == "ng2vc.py" else None)
    assert simulator._resolve_ng2vc({"ng2vc_binary": ""}) == found
...
def test_discovers_vacask_lib_layout(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(simulator.shutil, "which", lambda _n: None)  # nothing on PATH
    bindir = tmp_path / "bin"
    bindir.mkdir()
    vacask = _write(bindir / "vacask", "#!/bin/sh\n")
    pydir = tmp_path / "vacask" / "lib" / "vacask" / "python"
    pydir.mkdir(parents=True)
    ng2vc = _write(pydir / "ng2vc.py", "x")

    cfg = {"ng2vc_binary": "", "vacask_binary": vacask}
    assert simulator._resolve_ng2vc(cfg) == ng2vc


def test_resolve_none_when_nothing_found(monkeypatch) -> None:
    monkeypatch.setattr(simulator.shutil, "which", lambda _n: None)
    assert simulator._resolve_ng2vc({"ng2vc_binary": "", "vacask_binary": "vacask"}) is None


def test_argv0_python_vs_native() -> None:
    assert simulator._ng2vc_argv0("/x/ng2vc.py") == [sys.executable, "/x/ng2vc.py"]
    assert simulator._ng2vc_argv0("/x/ng2vc") == ["/x/ng2vc"]


# ── _run_ng2vc invocation (both conventions) ──────────────────────────────────

def _patch_cfg(monkeypatch, ng2vc_path: str) -> None:
    monkeypatch.setattr(simulator.app_config, "load_config",
...
def test_run_uses_output_file_form(monkeypatch, tmp_path, spice_in) -> None:
    ...
    simulator._run_ng2vc(spice_in, sim_file)
...
def test_run_surfaces_real_error(monkeypatch, tmp_path, spice_in) -> None:
    ...
    with pytest.raises(RuntimeError, match="boom: bad netlist"):
        simulator._run_ng2vc(spice_in, str(tmp_path / "tb.sim"))
